...
with st.sidebar:
    ...
    with st.expander("📊 插件生态概览（模拟）", expanded=False):
        # 模拟数据生成
        months = pd.date_range(end=pd.Timestamp.now(), periods=12, freq='M').strftime('%Y-%m').tolist()
        
        # 指数增长模型
        x = np.linspace(1, 12, 12)
        users = [int(50 * np.exp(0.4 * i)) for i in x]  # 用户数指数增长
        plugins = [int(u * 0.25 + np.random.randint(-10, 10)) for u in users] # 插件数为用户数的25%左右
        developers = [int(u * 0.08 + np.random.randint(-2, 2)) for u in users] # 开发者为用户数的8%左右
        
        df_ecosystem = pd.DataFrame({
            "月份": months,
            "注册用户": users,
            "DIY插件累计": plugins,
            "活跃开发者": developers
        ...
        current_plugins = plugins[-1]
        growth_rate = ((users[-1] - users[-2]) / users[-2]) * 100
        
        col_m1, col_m2 = st.columns(2)
        with col_m1:
            st.metric("当前总用户", f"{current_users:,}", f"+{growth_rate:.1f}%")
        with col_m2:
            st.metric("总插件数", f"{current_plugins:,}", f"+{int(growth_rate*0.9)}%")
            
        st.caption("活跃开发者: " + str(developers[-1]))
        
        # 趋势图
        st.markdown("##### 生态增长趋势")
        st.line_chart(df_ecosystem, color=["#00FFFF", "#FF00FF", "#9D00FF"]) # Cyan, Magenta, Purple
        
        st.caption("ℹ️ 基于增长模型的模拟数据，展示插件生态爆发潜力；未来真实上线后替换为实际统计。")
